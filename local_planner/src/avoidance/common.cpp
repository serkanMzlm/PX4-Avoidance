...
bool isInWhichFOV(const std::vector<FOV> &fov_vec, const PolarPoint &p_pol, int &idx)
{
  ...
  for (size_t i = 0; i < fov_vec.size(); ++i)
  {
    ...
    {
      if (retval)
      { // if it's been found before, return false!
        idx = -1;
        return false;
      }
      idx = i;
...
bool isOnEdgeOfFOV(const std::vector<FOV> &fov_vec, const PolarPoint &p_pol, int &idx)
{
  ...
  if (isInWhichFOV(fov_vec, p_pol, idx))
  {
    ...
    { // half-resolution degrees to the left
      just_outside.z =
          wrapAngleToPlusMinus180(fov_vec[idx].yaw_deg - fov_vec[idx].h_fov_deg / 2.0f - (ALPHA_RES / 2.0f));
    }
...
float angleDifference(float a, float b)
{
  ...
  return angle >= 0.f ? (angle < 180.f) ? angle : angle - 360.f : (angle >= -180.f) ? angle
                                                                                    : angle + 360.f;
}
...
pcl::PointCloud<pcl::PointXYZ> removeNaNAndGetMaxima(pcl::PointCloud<pcl::PointXYZ> &cloud)
{
  ...
  if (i_x_max >= 0)
    maxima.push_back(cloud.points[i_x_max]);
  if (i_y_max >= 0)
    maxima.push_back(cloud.points[i_y_max]);
  if (i_z_max >= 0)
    maxima.push_back(cloud.points[i_z_max]);
  if (i_x_min >= 0)
    maxima.push_back(cloud.points[i_x_min]);
  if (i_y_min >= 0)
    maxima.push_back(cloud.points[i_y_min]);
  if (i_z_min >= 0)
    maxima.push_back(cloud.points[i_z_min]);
