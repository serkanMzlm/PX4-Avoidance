...
LocalPlannerNode::LocalPlannerNode() : spin_dt_(0.1), Node("local_planner_node"), tf_buffer_(5.f)
{
    ...
    timer_.vehicle_update = this->create_wall_timer(std::chrono::milliseconds(250), std::bind(&LocalPlannerNode::publishOffboardControlMode, this));
    timer_.main_function = this->create_wall_timer(std::chrono::milliseconds(10), std::bind(&LocalPlannerNode::mainCallback, this));
    timer_.cloud_transform = this->create_wall_timer(std::chrono::milliseconds(5), std::bind(&LocalPlannerNode::transformBuffer, this));
    timer_.cmd_loop = this->create_wall_timer(std::chrono::milliseconds(100), std::bind(&LocalPlannerNode::cmdLoopCallback, this));
...
void LocalPlannerNode::initTopic()
{
    ...
    sub.camera_point_cloud = this->create_subscription<sensor_msgs::msg::PointCloud2>("/realsense_d435i/points", 100,
...
void LocalPlannerNode::printPointInfo(double x, double y, double z)
{
    ...
    int beta_z = floor((atan2(x - drone_pos.x(), y - drone_pos.y()) * D2R));
    int beta_e = floor((atan((z - drone_pos.z()) / (Eigen::Vector2f(x, y) - drone_pos.topRows<2>()).norm()) * D2R));

    beta_z = beta_z + (ALPHA_RES - beta_z % ALPHA_RES);
    beta_e = beta_e + (ALPHA_RES - beta_e % ALPHA_RES);
    RCLCPP_INFO(this->get_logger(), "Local Pose: %.2f, %.2f, %.2f", drone_pos.x(), drone_pos.y(), drone_pos.z());
    RCLCPP_INFO(this->get_logger(), "SetPoint: %f %f %f", x, y, z);
    RCLCPP_INFO(this->get_logger(), "Elevation %d Azimuth %d", beta_e, beta_z);
}
...
void LocalPlannerNode::mainCallback()
{
    ...
    auto loop_time = std::chrono::steady_clock::now() - start_time;
    auto required_delay = std::chrono::duration<double>(spin_dt_) - loop_time;

    if (required_delay > std::chrono::duration<double>(0))
    {
        std::this_thread::sleep_for(required_delay);
    }
}

void LocalPlannerNode::transformBuffer()
{
    {
        std::lock_guard<std::mutex> guard(buffered_transforms_mutex_);
...
void LocalPlannerNode::odomCallback(const px4_msgs::msg::VehicleOdometry::SharedPtr msg)
{
    ...
    RCLCPP_DEBUG(this->get_logger(), "local pose: %.2f, %.2f, %.2f", newest_position_.x(), newest_position_.y(), newest_position_.z());
    RCLCPP_DEBUG(this->get_logger(), "local orientation: %.2f, %.2f, %.2f, %.2f", newest_orientation_.x(), newest_orientation_.y(), newest_orientation_.z(), newest_orientation_.w());
}
...
void LocalPlannerNode::vehicleStatusCallback(const px4_msgs::msg::VehicleStatus::SharedPtr msg)
{
    ...
    case 3:
        ...
        RCLCPP_DEBUG(this->get_logger(), "MISSION");
        break;
    case 4:
        nav_state_ = NavigationState::auto_loiter;
        RCLCPP_DEBUG(this->get_logger(), "AUTO LOITER");
        break;
    case 5:
        nav_state_ = NavigationState::auto_rtl;
        RCLCPP_DEBUG(this->get_logger(), "AUTO RTL");
        break;
    case 14:
    ...
    case 17:
        ...
        RCLCPP_DEBUG(this->get_logger(), "AUTO TAKEOFF");
        break;
    case 18:
        nav_state_ = NavigationState::auto_land;
        RCLCPP_DEBUG(this->get_logger(), "AUTO LAND");
        break;
