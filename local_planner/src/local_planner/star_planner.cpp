...
#include "local_planner/tree_node.hpp"
#include "local_planner/star_planner.hpp"
...
void StarPlanner::setClosestPointOnLine(const Eigen::Vector3f &closest_pt)
{
...
void StarPlanner::buildLookAheadTree()
{
    ...
    for (int n = 0; n < n_expanded_nodes_ && is_expanded_node; n++)
    {
        ...
        histogram.setZero();
        generateNewHistogram(histogram, origin_position, cloud_);
    ...
    RCLCPP_INFO(rclcpp::get_logger("star_planner"), "\033[0;35m[SP]Tree (%lu nodes, %lu path nodes, %lu expanded) calculated in %2.2fms.\033[0m", tree_.size(),
